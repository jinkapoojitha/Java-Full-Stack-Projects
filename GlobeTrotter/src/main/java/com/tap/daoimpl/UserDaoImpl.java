...
public class UserDaoImpl implements UserDao {
    ...
    private static final String getUserStatusQuery = "SELECT status FROM users WHERE userId = ?";
    private static final String updateUserStatusQuery = "UPDATE users SET status = ? WHERE userId = ?";

    static {
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            con = DriverManager.getConnection("jdbc:mysql://localhost:3306/sevwonders", "root", "root");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
